...
export function ProfessorDataTable<TData, TValue>({
    ...
    const [minRating, setMinRating] = React.useState(0);   // 0..5
    const [maxDiff, setMaxDiff] = React.useState(5);       // 1..5

    const filteredData = React.useMemo(() => {
        return data.filter((row: any) => {
            const r = Number.parseFloat(row?.avg_rating ?? "0");
            const d = Number.parseFloat(row?.avg_difficulty ?? "0");
            const ratingOK = Number.isFinite(r) && r >= minRating;
            const diffOK   = Number.isFinite(d) && d <= maxDiff;
            return ratingOK && diffOK;
        });
    ...
    return (
        <div className="flex flex-col items-center text-center bg-background">
            <div className="flex flex-col items-center text-center bg-background w-full">
                ...
                {/* Rating / Difficulty sliders */}
                <div className="w-full max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-6 md:gap-10 px-4 mb-5">
                    {/* Min Avg Rating */}
